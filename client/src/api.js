...
api.interceptors.response.use(
    ...
    async (error) => {
        ...
        const isFail = !response || response.status >= 400 || error.code === 'ERR_NETWORK';
        ...
        if (isFail && isDemo()) {
            ...
            else if (url.includes('/projects/upload') || url.includes('/github/deploy')) mock = { id: 'demo', status: 'Live' };
            else if (url.includes('/projects')) mock = [{ id: '1', name: 'Alpha Portal', status: 'Live', url: '#' }];
